...
static void lcd_gc9a01_init_reg(void)
{
	LCD_WRITE_CMD(0xEF);
	LCD_WRITE_CMD(0xEB);
	LCD_WRITE_DATA(0x14);
	...
	else if(USE_HORIZONTAL==2)LCD_WRITE_DATA(0x68);
...
esp_err_t lcd_gc9a01_init(const scr_controller_config_t *lcd_conf)
{
    ...
    ret = lcd_gc9a01_set_rotation(lcd_conf->rotate);
    LCD_CHECK(ESP_OK == ret, "Set rotate failed", ESP_FAIL);
    ret = lcd_gc9a01_set_invert(1);  /**< GC9A01 setting the reverse color is the normal color */
